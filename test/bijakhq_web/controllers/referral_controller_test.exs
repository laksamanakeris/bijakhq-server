defmodule BijakhqWeb.ReferralControllerTest do
  ...
  describe "index" do
    test "lists all referrals", %{conn: conn} do
      conn = get conn, api_referral_path(conn, :index)
  ...
  describe "create referral" do
    test "renders referral when data is valid", %{conn: conn} do
      conn = post conn, api_referral_path(conn, :create), referral: @create_attrs
      assert %{"id" => id} = json_response(conn, 201)["data"]

      conn = get conn, api_referral_path(conn, :show, id)
      assert json_response(conn, 200)["data"] == %{
        "id" => id,
        "referred_by" => 42,
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post conn, api_referral_path(conn, :create), referral: @invalid_attrs
  ...
  describe "update referral" do
    ...
    test "renders referral when data is valid", %{conn: conn, referral: %Referral{id: id} = referral} do
      conn = put conn, api_referral_path(conn, :update, referral), referral: @update_attrs
    ...
    #   conn = put conn, api_referral_path(conn, :update, referral), referral: @invalid_attrs
    #   assert json_response(conn, 422)["errors"] != %{}
    # end
  end
  ...
  describe "delete referral" do
    ...
    test "deletes chosen referral", %{conn: conn, referral: referral} do
      conn = delete conn, api_referral_path(conn, :delete, referral)
      assert response(conn, 204)
      assert_error_sent 404, fn ->
        get conn, api_referral_path(conn, :show, referral)
      end
    end
  end
